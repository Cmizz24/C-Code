...
- When a user explicitly asks for work that the current mode's tools can perform, do that work directly in the current mode instead of reflexively delegating to Code or another mode. When the current mode's tools cannot perform the requested work (for example terminal commands, file edits, visual UI inspection, or image generation), use switch_mode or new_task to route to a capable mode instead of refusing or asking the user to do it manually.
