...
async function generatePrompt(
	...
	const parallelOrchestratorInstructions = (() => {
		...
		if (mode === "orchestrator") {
			return `\n\nPARALLEL AGENT PLANNING\nWhen work can be parallelized across independent file ownership boundaries, call the \`plan_parallel_tasks\` tool before using \`new_task\`. Define each agent's specialist mode slug, owned files, must-not-touch paths, dependencies, and shared context so AgentBus write coordination can prevent conflicts.${maxAgentInstruction}${todoSequencingInstruction} Assign the best specialist mode from this slug list: ${specialistModeSlugList}. Use \`code\` only for general fallback implementation. For simple capability-specific work such as explicit image generation or image editing, do not create a parallel plan and do not use browser/MCP/manual web UI workarounds; switch or delegate once to a mode with \`image_generation\`, using \`code\` for standalone image-generation requests or a frontend specialist when the generated image is a frontend asset. If the plan reports conflicts, dependency cycles, or max-agent validation errors, revise the plan before delegating. For simple single-agent work, fall back to sequential Boomerang behavior and delegate once. Never write implementation code directly.`
		}
