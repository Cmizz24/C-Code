...
const SWITCH_MODE_DESCRIPTION = `Request to switch to a different mode. Use this when the user's task should continue in the current conversation but requires a capability or tool group that the current mode does not have, rather than refusing or telling the user to do it manually. Do not request a mode switch when the current mode can complete the work with its available tools; avoid reflexive switches to Code. Do not switch modes just to inspect the workspace with read_file, search_files, or list_files because those read-only tools are available in every mode unless disabled. Examples: switch to Code for implementation or file edits only from modes that cannot edit, Debug for troubleshooting commands, CLI Tools for pure command-line work when available, a visual-inspection mode for visual_browser_inspector, or an image-capable mode for image_generation. For standalone image-generation requests from a mode without generate_image, switch to Code or another image-capable specialist rather than using browser/MCP/manual web UI workarounds. The user must approve the mode switch.`
