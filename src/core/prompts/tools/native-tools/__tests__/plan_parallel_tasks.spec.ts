...
describe("createPlanParallelTasksTool", () => {
	...
	it("guides plans toward shared contracts instead of unnecessary completion blockers", () => {
		...
		expect(tool.function.description).toContain("generated support/helper/library files")
		expect(tool.function.description).toContain("exactly one non-read-only owns scope")
		...
		expect(tool.function.description).toContain("dependencies do not prevent an agent from starting or completing")
		expect(tool.function.description).toContain("must not be used as hard scheduling or completion barriers")
		expect(tool.function.description).toContain("do not put those tasks in parallel")
		expect(tool.function.description).toContain("exact targetAgentId and waitForAnswer=true")
		...
		expect(agentProperties.dependsOn.description).toContain("do not prevent this agent from starting or completing")
		expect(agentProperties.dependsOn.description).toContain("not hard scheduling/completion barriers")
		expect(agentProperties.dependsOn.description).toContain("do not model it with dependsOn")
		expect(agentProperties.dependsOn.description).toContain("exact targetAgentId and waitForAnswer=true")
