...
The configured provider and model must actually support image generation. Use real image-generation providers such as OpenRouter image-output models, OpenAI/OpenAI-compatible Images API models, or Cloudflare Workers AI image models. Vision or image-understanding chat models are not valid image-generation models, and Ollama, LM Studio, ComfyUI, or Automatic1111 are not selectable image-generation providers in this tool flow. Use an edit-capable image provider when an input image needs to be transformed.
