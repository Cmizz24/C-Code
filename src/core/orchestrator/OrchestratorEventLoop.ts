...
export class OrchestratorEventLoop {
	...
	private buildAgentMessage(agent: AgentPlan, plan?: ExecutionPlan): string {
		...
		return [
			...
			"Before your first write, use coordinate_agents to read recent team chat. If there is an open question for you, answer it before writing when relevant. Otherwise ask one specific relevant agent one short integration question if you need a hook, selector, variable, data attribute, public function, or file contract.",
			"Before attempt_completion, read team chat again. If an answer arrived for a question you asked, adapt your files or final result around the answered hook, selector, variable, data attribute, public function, file contract, or user-facing name before finishing.",
			...
			"Use coordinate_agents like simple team chat: ask one relevant agent one short question at a time with targetAgentId where possible; answer with only the key hook, selector, variable, file, or decision needed and include replyToId from the question.",
	...
	private buildSystemPromptSuffix(agent: AgentPlan, plan?: ExecutionPlan): string {
		...
		return [
			...
			"- Before your first write, call coordinate_agents with action=read. If there is an open question for you, answer it before writing when relevant; otherwise ask one specific relevant agent one short integration question if you need a hook, selector, variable, data attribute, public function, or file contract.",
			"- Before attempt_completion, call coordinate_agents with action=read again. If an answer arrived for a question you asked, adapt your files or final result around the answered hook, selector, variable, data attribute, public function, file contract, or user-facing name before finishing.",
			"- Do not post ownership or introduction messages such as 'I own <file>' or 'Agent <id> owns <file>'. coordinate_agents publish is for real question/answer coordination only.",
			"- Use coordinate_agents as a concise team chat: ask one relevant agent one short question at a time with targetAgentId where possible, and answer with only the key hook, selector, variable, file, or decision needed. Answers should include replyToId from the question.",
