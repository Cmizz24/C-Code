...
export class OrchestratorEventLoop {
	...
	private buildAgentMessage(agent: AgentPlan, plan?: ExecutionPlan): string {
		...
		return [
			...
			"Publish a coordinate_agents question only when a blocking or unclear integration detail cannot be inferred from the task context. Answer targeted open questions concisely with replyToId when available.",
			...
			"Do not post pre-planned, basic, or filler questions just to populate team chat. Ask one relevant agent one short blocking question at a time with targetAgentId where possible; answer with only the key hook, selector, variable, file, or decision needed.",
	...
	private buildSystemPromptSuffix(agent: AgentPlan, plan?: ExecutionPlan): string {
		...
		return [
			...
			"- Use coordinate_agents only for genuine live coordination: read when you need current coordination state, need to answer an open targeted question, or need to check whether an answer to your own question arrived.",
			"- Publish a coordinate_agents question only when a blocking or unclear integration detail cannot be inferred from the task context. Answer targeted open questions concisely with replyToId when available.",
			"- If you read an answer to your own question, adapt your files or final result around the answered hook, selector, variable, data attribute, public function, file contract, or user-facing name before finishing.",
			"- Do not post ownership or introduction messages such as 'I own <file>' or 'Agent <id> owns <file>'. coordinate_agents publish is for real question/answer coordination only.",
			"- Do not post pre-planned, basic, or filler questions just to populate team chat. Ask one relevant agent one short blocking question at a time with targetAgentId where possible, and answer with only the key hook, selector, variable, file, or decision needed.",
