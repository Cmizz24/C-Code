...
export class OrchestratorEventLoop {
	...
	private buildAgentMessage(agent: AgentPlan, plan?: ExecutionPlan): string {
		...
		return [
			...
			"Before your first write, use coordinate_agents to read recent team chat. If there is an open question for you, answer it before writing when relevant. Otherwise ask one relevant agent one short integration question if you need a hook, selector, variable, data attribute, public function, or file contract.",
	...
	private buildSystemPromptSuffix(agent: AgentPlan, plan?: ExecutionPlan): string {
		...
		return [
			...
			"- Before your first write, call coordinate_agents with action=read. If there is an open question for you, answer it before writing when relevant; otherwise ask one relevant agent one short integration question if you need a hook, selector, variable, data attribute, public function, or file contract.",
			"- Do not post ownership or introduction messages such as 'I own <file>' or 'Agent <id> owns <file>'. coordinate_agents publish is for real question/answer coordination only.",
