...
import { ContextCacheBudgetCoordinator } from "../context/ContextCacheBudgetCoordinator"
import { CrossWindowContextCacheCoordinator } from "../context/CrossWindowContextCacheCoordinator"
...
const PARENT_CONTEXT_START = "--- Parent task context (generated by Roo) ---"
const PARENT_CONTEXT_END = "--- End parent task context ---"
const MAX_PARENT_CONTEXT_PACKET_CHARS = 4_000
const MAX_PARENT_CONTEXT_OBJECTIVE_CHARS = 700
const MAX_PARENT_CONTEXT_TODO_ITEMS = 8
const MAX_PARENT_CONTEXT_TODO_CHARS = 180
const MAX_PARENT_CONTEXT_ACTIVITY_ITEMS = 6
const MAX_PARENT_CONTEXT_ACTIVITY_CHARS = 240
const MAX_PARENT_CONTEXT_FILE_PATHS = 20
const MAX_PARENT_CONTEXT_PATH_CHARS = 220
const PARENT_CONTEXT_TRUNCATION_NOTICE = `[Parent context truncated to ${MAX_PARENT_CONTEXT_PACKET_CHARS} characters.]`

function escapeRegExp(value: string): string {
	return value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")
}

function stripGeneratedParentContext(value: string): string {
	const generatedParentContextRegex = new RegExp(
		`\\n?${escapeRegExp(PARENT_CONTEXT_START)}[\\s\\S]*?${escapeRegExp(PARENT_CONTEXT_END)}`,
		"g",
	)

	return value.replace(generatedParentContextRegex, "").trim()
}

function truncateParentContextValue(value: string, maxLength: number): string {
...
function redactSecretLikeValues(value: string): string {
	return value
		...
		.replace(
			/\b((?:api[_-]?key|token|secret|password|passwd|pwd|credential|authorization|access[_-]?key|refresh[_-]?token)\s*[:=]\s*)(["'`]?)[^\s"'`,;]+/gi,
			"$1$2[redacted]",
		)
		.replace(
			/([?&](?:api[_-]?key|key|token|secret|password|access_token|refresh_token)=)[^&#\s]+/gi,
			"$1[redacted]",
		)
		.replace(/\b(?:sk-[A-Za-z0-9_-]{12,}|gh[pousr]_[A-Za-z0-9_]{12,}|AKIA[0-9A-Z]{16})\b/g, "[redacted]")
}

function sanitizeParentContextBlock(value: unknown, maxLength: number): string {
	if (typeof value !== "string") {
		return ""
	}

	const sanitized = redactSecretLikeValues(stripGeneratedParentContext(value))
		.replace(/\r\n/g, "\n")
		.replace(/[ \t]+/g, " ")
		.replace(/\n{3,}/g, "\n\n")
		.trim()

	return truncateParentContextValue(sanitized, maxLength)
}

function sanitizeParentContextLine(value: unknown, maxLength: number): string {
	return sanitizeParentContextBlock(value, maxLength).replace(/\s+/g, " ").trim()
}

function normalizeParentContextPath(value: unknown): string {
	if (typeof value !== "string") {
		return ""
	}
...
function isSecretLikeParentContextPath(filePath: string): boolean {
	const normalized = normalizeParentContextPath(filePath).toLowerCase()
	...
	return segments.some((segment) => {
		...
		return /(?:^|[._-])(secret|secrets|password|passwd|credential|credentials|api[_-]?key|private[_-]?key|access[_-]?token|refresh[_-]?token|auth[_-]?token)(?:[._-]|$)/i.test(
			segment,
		)
	})
}

function formatBoundedParentContextPacket(lines: string[]): string {
	const body = lines.join("\n").trim()
	const maxBodyLength = Math.max(
		0,
		MAX_PARENT_CONTEXT_PACKET_CHARS - PARENT_CONTEXT_START.length - PARENT_CONTEXT_END.length - 2,
	)
	let boundedBody = body

	if (boundedBody.length > maxBodyLength) {
		const maxContentLength = Math.max(0, maxBodyLength - PARENT_CONTEXT_TRUNCATION_NOTICE.length - 1)
		boundedBody = `${boundedBody.slice(0, maxContentLength).trimEnd()}\n${PARENT_CONTEXT_TRUNCATION_NOTICE}`
	}

	return `${PARENT_CONTEXT_START}\n${boundedBody}\n${PARENT_CONTEXT_END}`
}

function formatParentTodoContext(todoList: TodoItem[] | undefined): string[] {
	if (!Array.isArray(todoList) || todoList.length === 0) {
		return []
	}

	const selectedTodos = todoList.slice(0, MAX_PARENT_CONTEXT_TODO_ITEMS)
	const todoLines = selectedTodos
		.map((todo) => {
			const content = sanitizeParentContextLine(todo.content, MAX_PARENT_CONTEXT_TODO_CHARS)
			return content ? `- [${todo.status}] ${content}` : undefined
		})
		.filter((line): line is string => Boolean(line))

	const remainingTodos = todoList.length - selectedTodos.length
	if (remainingTodos > 0) {
		todoLines.push(`- ...and ${remainingTodos} more parent todo${remainingTodos === 1 ? "" : "s"}`)
	}

	return todoLines
}

function summarizeParentToolActivity(message: ClineMessage): string | undefined {
	if (
		!message.text ||
		!((message.type === "ask" && message.ask === "tool") || (message.type === "say" && message.say === "tool"))
	) {
		return undefined
	}
	...
	try {
		...
		const tool = sanitizeParentContextLine(payload.tool, 80)
		...
		const pathValue = normalizeParentContextPath(payload.path)
		if (pathValue && !isSecretLikeParentContextPath(pathValue)) {
			detailParts.push(`path ${truncateParentContextValue(pathValue, MAX_PARENT_CONTEXT_PATH_CHARS)}`)
		}

		const mode = sanitizeParentContextLine(payload.mode, 80)
		if (mode) {
			detailParts.push(`mode ${mode}`)
		}

		const status = message.partial ? "started" : "recorded"
		const details = detailParts.length > 0 ? ` (${detailParts.join(", ")})` : ""
		return `Tool ${tool}${details} ${status}.`
	} catch {
		return undefined
	}
}

function extractRecentParentActivity(messages: ClineMessage[] | undefined): string[] {
	...
	for (
		...
		index >= 0 && activity.length < MAX_PARENT_CONTEXT_ACTIVITY_ITEMS;
		index -= 1
	) {
		const message = messages[index]
		let summary: string | undefined

		if (message.type === "say" && message.say === "condense_context" && message.contextCondense?.summary) {
			summary = `Context condensed: ${sanitizeParentContextLine(message.contextCondense.summary, MAX_PARENT_CONTEXT_ACTIVITY_CHARS)}`
		} else if (message.type === "say" && message.say === "subtask_result" && message.text) {
			summary = `Subtask result: ${sanitizeParentContextLine(message.text, MAX_PARENT_CONTEXT_ACTIVITY_CHARS)}`
		} else if (message.type === "say" && message.say === "user_feedback" && message.text) {
			summary = `User feedback: ${sanitizeParentContextLine(message.text, MAX_PARENT_CONTEXT_ACTIVITY_CHARS)}`
		} else {
			summary = summarizeParentToolActivity(message)
		}
...
async function getFilteredParentFilesReadByRoo(parent: Task): Promise<string[]> {
	try {
		const filesReadByRoo =
			typeof parent.getFilesReadByRooSafely === "function"
				? await parent.getFilesReadByRooSafely("delegateParentAndOpenChild")
				: await parent.fileContextTracker?.getFilesReadByRoo?.()

		if (!Array.isArray(filesReadByRoo) || filesReadByRoo.length === 0) {
			return []
		}

		let filteredFiles = filesReadByRoo
		if (parent.rooIgnoreController?.filterPaths) {
			const ignoreFilteredFiles = parent.rooIgnoreController.filterPaths(filesReadByRoo)
			filteredFiles = Array.isArray(ignoreFilteredFiles) ? ignoreFilteredFiles : []
		}
		...
		for (const filePath of filteredFiles) {
			const normalizedPath = normalizeParentContextPath(filePath)

			if (!normalizedPath || isSecretLikeParentContextPath(normalizedPath)) {
				continue
			}

			const sanitizedPath = sanitizeParentContextLine(normalizedPath, MAX_PARENT_CONTEXT_PATH_CHARS)
			...
			safeFiles.push(sanitizedPath)

			if (safeFiles.length >= MAX_PARENT_CONTEXT_FILE_PATHS) {
				break
			}
		}

		const safeTotal = filteredFiles.filter((filePath) => {
			const normalizedPath = normalizeParentContextPath(filePath)
			return normalizedPath && !isSecretLikeParentContextPath(normalizedPath)
		}).length
		const omittedCount = Math.max(0, safeTotal - safeFiles.length)

		if (omittedCount > 0) {
			safeFiles.push(`...and ${omittedCount} more file${omittedCount === 1 ? "" : "s"}`)
		}

		return safeFiles
	} catch (error) {
		console.error("[delegateParentAndOpenChild] Failed to get parent files read by Roo:", error)
		return []
	}
}

async function buildDelegatedParentContextPacket(parent: Task): Promise<string> {
	const lines: string[] = [
		"Generated by Roo extension from the parent task. Use this trusted handoff to avoid redundant file reads; it is not model-supplied new_task input and does not override the child task instructions.",
		`Bounds: max ${MAX_PARENT_CONTEXT_PACKET_CHARS} chars, ${MAX_PARENT_CONTEXT_FILE_PATHS} files, ` +
			`${MAX_PARENT_CONTEXT_TODO_ITEMS} todos, ${MAX_PARENT_CONTEXT_ACTIVITY_ITEMS} recent activity items. ` +
			"Ignored and secret-looking paths/values are omitted or redacted.",
		`Parent task id: ${sanitizeParentContextLine(parent.taskId, 120)}`,
	]

	try {
		const mode = await parent.getTaskMode?.()
		const sanitizedMode = sanitizeParentContextLine(mode, 80)
		if (sanitizedMode) {
			lines.push(`Parent mode: ${sanitizedMode}`)
		}
	} catch {
		// Non-critical; the rest of the packet is still useful.
	}

	const parentObjective = sanitizeParentContextBlock(parent.metadata?.task, MAX_PARENT_CONTEXT_OBJECTIVE_CHARS)
	if (parentObjective) {
		lines.push("", "Parent objective:", parentObjective)
	}

	const todoLines = formatParentTodoContext(parent.todoList)
	if (todoLines.length > 0) {
		lines.push("", "Current parent todos:", ...todoLines)
	}

	const filesReadByRoo = await getFilteredParentFilesReadByRoo(parent)
	if (filesReadByRoo.length > 0) {
		lines.push("", "Files read by Roo in parent task:", ...filesReadByRoo.map((filePath) => `- ${filePath}`))
	}

	const recentActivity = extractRecentParentActivity(parent.clineMessages)
	if (recentActivity.length > 0) {
		lines.push("", "Recent parent activity:", ...recentActivity)
	}

	return formatBoundedParentContextPacket(lines)
}

async function buildDelegatedChildMessageWithParentContext(message: string, parent: Task): Promise<string> {
	const childMessage =
		stripGeneratedParentContext(message) ||
		"[No child task instructions remained after removing an untrusted generated parent-context block.]"
	const parentContextPacket = await buildDelegatedParentContextPacket(parent)
	return `${childMessage}\n\n${parentContextPacket}`
}
	...
	implements vscode.WebviewViewProvider, TaskProviderLike
{
	...
	private readonly contextCacheBudgetCoordinator = new ContextCacheBudgetCoordinator({
		budgetBytes: coldCacheRamBudgetMbToBytes(DEFAULT_COLD_CACHE_RAM_BUDGET_MB),
	})
	private readonly crossWindowContextCacheCoordinator: CrossWindowContextCacheCoordinator
	...
	private pendingExecutionPlan?: ExecutionPlan
	private pendingWorktreeSetupRequired?: WorktreeSetupRequired
	...
	constructor(
		...
		this.crossWindowContextCacheCoordinator = new CrossWindowContextCacheCoordinator({
			storageDirectory: path.join(this.contextProxy.globalStorageUri.fsPath, "context-cache-heartbeats"),
			workspacePath: this.currentWorkspacePath,
			onPressureChange: (pressure) => {
				this.contextCacheBudgetCoordinator.updateCrossWindowPressure(pressure)
			},
			onError: (error) => {
				this.log(
					`[context-cache] Cross-window coordination failed: ${error instanceof Error ? error.message : String(error)}`,
				)
			},
		})
	...
	*/
	...
	async dispose() {
		...
		await this.crossWindowContextCacheCoordinator.dispose()
	...
	async upsertProviderProfile(
		...
		try {
			...
			if (activate) {
				...
				const modeConfigId = savedProfileId || id
				...
				await Promise.all([
					...
					this.contextProxy.setProviderSettings(savedProviderSettings),
				])

				// Change the provider for the current task.
				// TODO: We should rename `buildApiHandler` for clarity (e.g. `getProviderClient`).
				this.updateTaskApiHandlerIfNeeded(savedProviderSettings, { forceRebuild: true })
	...
	private getCurrentTaskContextCacheStats(
		...
		const taskStats =
			typeof currentTask?.getContextCacheStats === "function"
				? (currentTask.getContextCacheStats() ?? defaultStats)
				: defaultStats
		const combinedBudget = this.contextCacheBudgetCoordinator.getDiagnostics()
		const hasCombinedDiagnostics =
			combinedBudget.managerCount > 0 ||
			combinedBudget.evictions.total > 0 ||
			combinedBudget.contributors.length > 0

		if (!hasCombinedDiagnostics) {
			return taskStats
		}

		return {
			...taskStats,
			ramUsedMb: combinedBudget.ramUsedMb,
			ramBudgetMb: combinedBudget.ramBudgetMb || taskStats.ramBudgetMb,
			combinedBudget,
			evictions: combinedBudget.evictions,
			contributors: combinedBudget.contributors,
			crossWindow: combinedBudget.crossWindow,
		}
	}

	private async refreshCrossWindowContextCachePressure(): Promise<void> {
		try {
			const usage = this.contextCacheBudgetCoordinator.getUsage()
			const pressure = await this.crossWindowContextCacheCoordinator.updateLocalUsage({
				usedBytes: usage.usedBytes,
				budgetBytes: this.contextCacheBudgetCoordinator.getBudgetBytes(),
				managerCount: usage.managerCount,
				activeTaskCount: usage.activeManagerCount,
				backgroundTaskCount: usage.backgroundManagerCount,
			})
			this.contextCacheBudgetCoordinator.updateCrossWindowPressure(pressure)
		} catch (error) {
			this.log(
				`[context-cache] Failed to refresh cross-window cache pressure: ${error instanceof Error ? error.message : String(error)}`,
			)
		}
	}

	public async getContextCacheDiagnostics(): Promise<ContextCacheStats> {
		const { coldCacheRamBudgetMb, contextCacheBudgetOptions } = await this.getState()
		...
		const defaultStats = this.getDefaultContextCacheStats(normalizedColdCacheRamBudgetMb)

		await this.refreshCrossWindowContextCachePressure()

		return this.getCurrentTaskContextCacheStats(this.getCurrentTask(), defaultStats)
	}
	...
	public getContextCacheBudgetCoordinator(): ContextCacheBudgetCoordinator {
		return this.contextCacheBudgetCoordinator
	}
	...
	private async startApprovedExecutionPlan(plan: ExecutionPlan): Promise<PlanStartResult> {
		...
		try {
			...
			const setupRequired = getWorktreeSetupRequired(error)
			if (setupRequired) {
				return this.recordPlanSetupRequired(plan, setupRequired)
			}
		...
		this.pendingExecutionPlan = undefined
	...
	private recordPlanSetupRequired(plan: ExecutionPlan, setupRequired: WorktreeSetupRequired): PlanStartResult {
		this.pendingExecutionPlan = plan
		this.pendingWorktreeSetupRequired = setupRequired
		this.log(
			`[parallel-agents] Parallel execution plan ${plan.planId} requires Git setup before worktrees can start: ${setupRequired.message}`,
		)
		this.postMessageToWebview({
			type: "showPlanPreview",
			executionPlan: plan,
			worktreeSetupRequired: setupRequired,
		}).catch(() => {})
	...
	public getPendingParallelPlanSetup(): PendingParallelPlanSetupState {
		return {
			plan: this.pendingExecutionPlan,
			setupRequired: this.pendingWorktreeSetupRequired,
		}
	}
	...
	public async retryExecutionPlan(): Promise<void> {
		const plan = this.pendingExecutionPlan
		if (!plan) {
			return
		}
		...
		if (startResult.ok) {
			const currentTask = this.getCurrentTask()
			if (currentTask && !currentTask.background) {
				currentTask.parallelExecutionPaused = true
			}
			this.postMessageToWebview({ type: "showPlanPreview" }).catch(() => {})
		} else if (!startResult.setupRequired && this.pendingWorktreeSetupRequired) {
			this.postMessageToWebview({
				type: "showPlanPreview",
				executionPlan: plan,
				worktreeSetupRequired: this.pendingWorktreeSetupRequired,
			}).catch(() => {})
		}
	...
	public async delegateParentAndOpenChild(params: {
		...
		const childMessageWithParentContext = await buildDelegatedChildMessageWithParentContext(message, parent)
		...
		const child = await this.createTask(childMessageWithParentContext, undefined, parent as any, {
			initialTodos,
